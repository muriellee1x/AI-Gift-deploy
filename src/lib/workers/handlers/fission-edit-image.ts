...
export async function handleFissionEditImageTask(job: Job<TaskJobData>) {
  ...
  if (!imageUrl) throw new UnrecoverableError('imageUrl is required')

  await reportTaskProgress(job, 10)

  const refBuffer = await fetchImageBuffer(imageUrl)

  await reportTaskProgress(job, 30)

  const result = await callUserImageAPI(userId, prompt, [refBuffer], apiConfigId, { resolution })
