...
export const POST = apiHandler(async (request: NextRequest) => {
  ...
  const { analysisAsset, selectedThemes, characterImagesDesc } = body as {
    analysisAsset?: Record<string, unknown>
    selectedThemes?: unknown[]
    characterImagesDesc?: string
  }
  ...
  const taskId = await createAndEnqueueTask({
    ...
    projectId: 'fission',
    type: TASK_TYPE.FISSION_VIDEO_PROMPTS,
    targetType: 'fission',
    targetId: 'video-prompts',
    payload: { analysisAsset, selectedThemes, characterImagesDesc: characterImagesDesc || '' },
  })
